          ...
          for (let i = 0; i < guess.letters.length; i++) {
            ...
            if (guessLetter.status === LetterStatus.Misplaced && !word.includes(guessLetter.char)) {
              if (guessLetter.status === LetterStatus.Misplaced) {
                const misplacedCountInGuess = guess.text
                  .split('')
                  .filter((c) => c === guessLetter.char).length
                const misplacedCountInWord = word
                  .split('')
                  .filter((c) => c === guessLetter.char).length
                const samePosition = guessLetter.char === word[i]

                if (
                  !word.includes(guessLetter.char) ||
                  misplacedCountInGuess !== misplacedCountInWord ||
                  samePosition
                ) {
                  lettersMatch = false
                  break
